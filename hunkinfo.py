...
class HunkReader(object):
    ...
    def _read_debug_block(self):
        ...
        # The content of a HUNK_DEBUG block was not specified by Commodore. We assume here
        # that it only contains a line / offset table in the format used by SAS/C. This
        # format is also generated by VBCC / VLINK and the code below is based on the function
        # linedebug_hunks() in the file t_amigahunk.c from VLINK.
        nwords_total = self._read_word()
        log(DEBUG, "size (in words) of debug block: %d", nwords_total)
        log(DEBUG, "section offset: 0x%08x", self._read_word())
        if self._read_string(4) != 'LINE':
            raise ValueError("format of HUNK_DEBUG block not recognized")
        nwords_fname = self._read_word()
        log(DEBUG, "file name: %s", self._read_string(nwords_fname * 4))
        nwords_total = nwords_total - nwords_fname - 3
        while nwords_total > 0:
            log(DEBUG, "line #%d at offset 0x%08x", self._read_word(), self._read_word())
            nwords_total -= 2
